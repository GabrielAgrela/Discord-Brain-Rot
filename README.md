...
- **Three-way branching** after transcription:
    ...
    3. **No command** (e.g. "ventura, you are useless"): the bot routes the transcript to **OpenRouter** (DeepSeek model) which generates an angry André Ventura parody reply in European Portuguese with ElevenLabs square-bracket performance tags. The reply is sent to **ElevenLabs Ventura TTS** and played back.
...
- **Mixed-language support**: The wake word may appear **anywhere** in the transcript (not only at the start), so Portuguese preamble such as "Mas que merda? Ventura, toca das páginas." is handled correctly. Only Portuguese command verbs (`toca`, `tocar`, `toque`, `mete`, `meter`, `põe`, `poe`, `reproduz`, `reproduzir`) are recognised and normalised to `"play"`. English `play` is **not** recognised as a Ventura voice command verb. Note: Whisper sometimes transcribes the spoken imperative `toca` as the formal/conjunctive `toque`, so `toque` is included as an alias.
...
- **Voice commands** are built on top of keyword detection: the `VOICE_COMMAND_WAKE_ALIASES` (default `ventura`) are injected into the Vosk grammar.
  ...
  - When a wake word is detected, recent audio from the speaking user is sent to **Groq Whisper** (`whisper-large-v3`) for transcription.
   - If the transcript contains a recognised Portuguese command verb (`toca`, `tocar`, `toque`, `mete`, `meter`, `põe`, `poe`, `reproduz`, `reproduzir`), the same fuzzy-matching playback path as `/toca` is executed via `SoundService.play_request`. English `play` is **not** recognised. If the argument matches an existing sound list, `SoundService.play_random_sound_from_list` is used instead.
...
- `VENTURA_CHAT_MODEL` (optional; OpenRouter model for Ventura chat replies, default `deepseek/deepseek-v4-flash`)
- `VENTURA_CHAT_REASONING_ENABLED` (optional; whether reasoning is enabled for the model, default `false`)
...
- `EL_TTS_LIVE_PREROLL_MS` (optional; explicit FFmpeg audio delay in milliseconds applied to the live TTS stream, default `120`; setting this lower reduces the speak delay but can clip the first words, while higher values improve playback reliability under jittery connections)
- `EL_TTS_LIVE_LOW_LATENCY_FFMPEG` (optional; use `-fflags nobuffer -flags low_delay` for live TTS stream decoding to bypass internal decoding buffer latency, default `false`; enabling it can start faster but may increase first-word clipping risk)
- `EL_TTS_LIVE_ASSUME_MP3_FORMAT` (optional; bypass FFmpeg's format probing delay by passing `-f mp3` explicitly when live output starts with `mp3_`, default `true`)
