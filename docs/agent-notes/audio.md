 ...
 - When `action == "voice_command"` in `trigger_action`, `_handle_voice_command` is called:
   ...
   8. **Else** (no recognised play command):
      ...
      - The transcript is sent to `VenturaChatService.reply()` (OpenRouter Qwen Coder model) which returns short European Portuguese text with ElevenLabs square-bracket performance tags.
       ...
       - `VenturaChatService` keeps an in-memory rolling history (up to 3 last user/assistant exchanges) per conversation key. The key format is `f"guild:{guild.id}:user:{user_id}"` (set in `KeywordDetectionSink._handle_voice_command`). Including history in OpenRouter payload allows follow-up queries to retain context. History is only appended on successful non-empty replies; API errors/timeouts/empty responses do not pollute history.
       - Before each POST, `VenturaChatService.reply()` logs the **full request payload** at INFO level with the `[VenturaChat] Request payload` prefix. The log entry includes the complete messages array (system prompt, historical user/assistant pairs, current user transcript) as pretty-printed JSON via `json.dumps(ensure_ascii=False, indent=2)` so Portuguese/accented text is readable in daily log files. No secrets (API keys, auth headers) are included in the log — the payload dict only contains model/temperature/max_tokens/messages.
