    ...
    8. **If** the parser returns `("mute", "")`:
     ...
     9. **Else** (no recognised play/mute command):
       ...
       - A `request_note` is computed from the transcript via `build_voice_request_note()` (strips the last wake word and trailing punctuation) so the generated TTS sound card shows the user's heard command as a ``TTS:`` footer pill — the same footer style used for play commands.
        ...
        - `VenturaChatService` keeps an in-memory rolling history (up to 3 last user/assistant exchanges) per conversation key. The key format is `f"guild:{guild.id}:user:{user_id}"` (set in `KeywordDetectionSink._handle_voice_command`). Including history in OpenRouter payload allows follow-up queries to retain context. By default, it always includes up to the last 3 prior exchanges. History is only appended on successful non-empty replies; API errors/timeouts/empty responses do not pollute history. The transcript sent to OpenRouter is prefixed with the requester's name as a speaker label (e.g. `Sopustos: <transcript>`) when available so the chat model knows who is speaking. This formatted form is also stored in the history.
...
- When enabled (default `true`), `save_as_mp3_EL` creates a POSIX FIFO and starts `AudioService.play_tts_live_stream()` as a background task **early (pre-started before making the ElevenLabs HTTP request)**. This allows voice client connectivity and FFmpeg startup to run concurrently with the ElevenLabs generation TTFB.
- To ensure the live task starts setting up immediately, `save_as_mp3_EL` yields to the event loop via `await asyncio.sleep(0)` right after creating the task.
...
- `FFmpegPCMAudio` startup uses a basic volume-only filter (`volume=1.0`) and conditionally appends `adelay={EL_TTS_LIVE_PREROLL_MS}:all=1` only if `EL_TTS_LIVE_PREROLL_MS` > 0 (defaults to `25` ms for extremely low latency with high reliability); no ear protection filters are applied.
