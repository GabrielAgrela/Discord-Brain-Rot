...
- Inbound recent-audio recording and speech-training work is suppressed while outbound playback is active by default (`AUDIO_SUPPRESS_RECORDING_WHILE_PLAYING=true`), but Vosk keyword detection must still run so commands such as `chapada` work during playback. Explicit `_active_captures` used by voice commands also bypass suppression.
