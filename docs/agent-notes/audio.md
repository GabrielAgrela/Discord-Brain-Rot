...
- Host system monitor per-process scans can take many seconds on the host and starve audio/event-loop work. `BackgroundService.web_system_monitor_status_loop()` must use `top_limit=0` while voice is occupied so cheap CPU/RAM snapshots continue without scanning every process.
