...
- Inbound `KeywordDetectionSink.write()` work is suppressed while outbound playback is active by default (`AUDIO_SUPPRESS_RECORDING_WHILE_PLAYING=true`), except for explicit `_active_captures` used by voice commands. Keep this guard when changing Vosk/speech-training receive paths; Discord receive bursts can otherwise compete with the voice-player thread and cause audible stalls.
