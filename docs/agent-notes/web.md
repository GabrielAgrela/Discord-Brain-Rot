...
- ``POST /api/speech_training/keyword_scan`` — start an **async** keyword scan job using offline Vosk.  Accepts JSON body with optional ``keyword`` (default ``"chapada"``), ``min_confidence`` (default ``0.5``), ``guild_id``, ``user_id``, and ``delete_non_matches`` (default ``false``).  When ``delete_non_matches`` is ``true``, scanned non-matching clips are deleted after the scan completes.  Returns ``202 {"job_id": ..., "status": "queued"}``.  Poll ``GET /api/speech_training/keyword_scan/<job_id>`` for progress and results.  Only unlabeled clips with ``duration_seconds <= 30`` are eligible; the response includes ``max_duration_seconds`` (always 30.0).
- ``GET /api/speech_training/keyword_scan/<job_id>`` — poll a keyword scan job.  Terminal states are ``done`` (includes ``matches[]``, ``scanned``, ``matched``, ``skipped``, ``max_duration_seconds``, ``delete_non_matches``, and ``deleted_non_matches``) and ``error`` (includes ``error`` message).  During processing the response includes ``total``, ``scanned``, ``matched``, ``skipped``, and ``max_duration_seconds`` for progress display.
...
- The JS polls ``GET /api/speech_training/transcribe_empty/<job_id>`` every 500 ms and shows a progress summary (e.g. ``5/20 · 3 updated · 1 empty · 1 skipped``).
